...
from scripts.manage_nft import deploy_and_create_nft, create_nft, last_nft
import time
...
def deploy_auction_from_factory(
    ...
    #
    #
    tx = factory.createSealedBidAuctionContract(
        initialHash, collectible, collectible_id, time1, time2, {"from": account}
    )
    tx.wait(1)
    #
    auction_address = factory.sealedBidAuctionArrayContracts(0)
    print(auction_address)
    auction = Contract.from_abi(
        SealedBidAuction._name, auction_address, SealedBidAuction.abi
    )
